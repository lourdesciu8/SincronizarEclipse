package EstacionDeTrenes;

import java.util.Random;
import java.util.Scanner;
import java.util.concurrent.Semaphore;

public class EstacionDeTrenes extends Thread{
	...
	protected static EstacionDeTrenes trenes[];
	...
	protected EstacionDeTrenes(int cantidadPasajeros, int cantidadCarga, int nombreTren) {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		//Se llama al arreglo
		trenes = new EstacionDeTrenes[cantidad_trenes_cargas + cantidad_trenes_pasajeros];
		...
		for(int i = 0; i < cantidad_trenes_cargas; i++) {
			...
			trenes[index] = new EstacionDeTrenes(0, cantidad_carga, index + 1);
			trenes[index].start();
			index++;
		}
		...
		for(int i = 0; i < cantidad_trenes_pasajeros; i++) {
			...
			trenes[index] = new EstacionDeTrenes(cantidad_pasajeros, 0, index + 1);
			trenes[index].start();
			index++;
		}
		
		//Es util para esperar que todos los trenes hayan hecho el recorrido por la estación
		for (EstacionDeTrenes tren: trenes) {
			tren.join();
		}
